...
function readCartItems() {
  ...
  try {
    const raw = toPlainSerializable(JSON.parse(localStorage.getItem('cart') || '[]')) || []
    return deduplicateCartItems(raw)
  } catch {
    return []
  }
}
...
export const useCartStore = defineStore('cart', {
  ...
  actions: {
    ...
    saveCart() {
      if (import.meta.server) return
      this.items = deduplicateCartItems(toPlainSerializable(this.items))
