...
const props = defineProps({
  /** Товар с SSR (Nuxt) — не перезапрашивать на первом рендере */
  ssrProduct: { type: Object, default: null },
})
...
function applyProductFromStore(p) {
  if (!p) return
  ...
  if (currentSku && Array.isArray(p.variants) && p.variants.length > 0) {
    ...
    if (idx !== -1) selectedVariantIndex.value = idx
  }
  const sku = selectedVariant.value?.sku
